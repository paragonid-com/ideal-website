...
> **ПОСЛЕДНЯЯ СЕССИЯ ЗАВЕРШЕНА (Сессия 17, 04.06.26): полировка главной + NAP/часы/футер/нейминг.**
...
>
> **ФОКУС СЛЕДУЮЩЕЙ СЕССИИ НЕ ЗАФИКСИРОВАН — спроси клиента.** Главные открытые
> направления: формы→backend (заявки сейчас теряются, `data-todo="form-submission"`);
> копирайт + комплаенс LegitScript (Lorem-тексты на peptide/iv/bloodwork/sexualhealth,
> FAQ везде Lorem); лицензии фото (Signature низкого разрешения, hero-watermark,
> stock на iv/bloodwork); домен-миграция `idealmedwell.com`→`idealmedical.com`
> (301-редиректы; оба сейчас live); локальный SEO + опц. лендинги по городам.
...
>
> Состояние проекта описано в `docs/HANDOFF.md` — прочитай его перед тем как что-то делать. Особенно верхний callout и раздел **«Сессия 17»** (последняя работа: NAP/Suite 345, виджет часов, соцсети+Cherry, переименование Regenerative Medicine, полировка главной).
>
> **ФОКУС ЭТОЙ СЕССИИ НЕ ЗАФИКСИРОВАН — спроси меня, что делаем, прежде чем начинать.** Меню/навигация (десктоп sticky-bar + mega-menu в сессии 16, мобильный drill-down ранее) и базовая подгонка главной по Figma — уже сделаны. Вероятные направления: формы→backend (заявки сейчас теряются), копирайт + комплаенс LegitScript (Lorem на peptide/iv/bloodwork/sexualhealth, FAQ везде Lorem), лицензии фото, домен-миграция `idealmedwell.com`→`idealmedical.com` (301), локальный SEO + лендинги по городам, или точечная полировка. Не предполагай — уточни приоритет.
>
