...
> **ПОСЛЕДНЯЯ СЕССИЯ ЗАВЕРШЕНА (Сессия 16): РЕДИЗАЙН ДЕСКТОП-НАВИГАЦИИ.**
> Десктоп-hamburger/overlay заменён на постоянную горизонтальную панель (`fixed`,
> поверх hero) + hover/click mega-menu на 3 категории. Панель авто-прячется при
> скролле вниз и появляется при скролле вверх; всегда белая (контраст AA); тень
> добавляется после прокрутки. Hero переработан (тексты/расположение/читаемость,
> фон остаётся на весь экран). Мобильное drill-down меню «Вариант C» — БЕЗ изменений.
> Подробности — раздел **«Сессия 16 — десктоп-навигация»** ниже.
> ⚠️ Три решения, требующие подтверждения клиента, — в начале того раздела.
>
> **ФОКУС СЛЕДУЮЩЕЙ СЕССИИ: МОБИЛЬНЫЙ АДАПТИВ + аудит CTA/секций.** Точки входа:
> навигация — `src/data/navigation.ts` (3 группы: Wellness Services 7, Aesthetic
> Services 4 [Emface/Emsculpt Neo/Exomind/Emsella], About Us 4 + `bookingCta`);
> вёрстка/логика меню — `src/components/Header.astro` (мобильный top-bar + burger
> `<lg`, десктоп-панель `#desktop-nav` `lg+`, мобильное drill-down «Вариант C»).
> См. готовый промпт в конце файла.
