...
> **ПОСЛЕДНЯЯ СЕССИЯ ЗАВЕРШЕНА (Сессия 18, 04–05.06.26): страница `hormone` доведена до Figma 1:1 — desktop + mobile.**
> Это эталон и набор паттернов для всех остальных сервисных страниц (см. «Сессия 18» и «ПЛЕЙБУК сервисных страниц» ниже).
> Предыдущая работа (Сессия 17): полировка главной + NAP/Suite 345/часы/футер/нейминг.
> После редизайна десктоп-навигации (Сессия 16) и выравнивания главной по Figma
> сделаны: sticky авто-скрытие мобильного top-bar; реальные фото Signature + 6-я
> карточка (mobile); **cache-bust фото** (новые имена файлов — иначе CDN/мобильный
> отдают старое по тому же URL); шрифт «We are located…» приведён к Figma (Manrope,
> без капса, узел 1:264); EMSELLA-карточка перекадрирована (устройство по центру).
> Затем — пакет по данным клиники: **Suite 345 в адресе ВЕЗДЕ** (видимая разметка +
> JSON-LD), **новые часы** (Mon–Fri 10:00–18:00, Sat By Appointment, Sun Closed),
> **сворачиваемый виджет часов** «Open today …» (новый `HoursWidget.astro`),
> **реальные соцсети** (FB/IG/YT) + **ссылка Cherry Financing** в футере,
> переименование **«PRP & Allograft» → «Regenerative Medicine»** (меню + футер +
> карточка). Подробности — раздел **«Сессия 17»** ниже. `origin/main` после
> применения всех патчей = `16c7963`.
>
> **ФОКУС СЛЕДУЮЩЕЙ СЕССИИ ЗАФИКСИРОВАН: обновить ВСЕ остальные сервисные страницы**
> по паттернам, отработанным на hormone. Методология — раздел **«ПЛЕЙБУК: как мы
> делаем/обновляем сервисные страницы»** ниже (плюс «Сессия 18» как эталон).
> Очередь: weight-loss, peptide, emsculpt, exomind, emface, emsella, ivtherapy,
> bloodwork, regenerative, sexualhealth. `origin/main` = `9fcfc90`. Готовый промпт —
> в конце файла. Прочие направления (формы→backend, комплаенс/копирайт LegitScript,
> лицензии фото, домен-миграция, локальный SEO) остаются в бэклоге ниже.
...
1. **Готово (эталон):** hormone — все секции, desktop + mobile.
2. **Очередь** (каждую: §5 геометрия → §3–4 desktop → §6 mobile → §8 аудит CTA →
   §4 реальные ассеты → §7 верификация+патч):
   weight-loss · peptide · emsculpt · exomind · emface · emsella · ivtherapy ·
   bloodwork · regenerative · sexualhealth.
3. Спросить Юри, с какой страницы начать (или предложить порядок — логично от
   самых посещаемых/готовых по контенту: weight-loss → peptide → device-страницы).
4. Параллельные блокеры (не блокируют вёрстку, но держать в голове): Lorem-копирайт
   на peptide/iv/bloodwork/sexualhealth + FAQ везде; лицензии stock-фото
   (iv/bloodwork); видео-URL (emsculpt/exomind/emface/emsella).
...
>
> Состояние проекта описано в `docs/HANDOFF.md` — прочитай его перед тем как что-то делать. Особенно верхний callout, раздел **«Сессия 18»** (hormone доведён до Figma 1:1, desktop+mobile) и **«ПЛЕЙБУК: как мы делаем/обновляем сервисные страницы»** — по нему работаем дальше.
>
> **ФОКУС ЭТОЙ СЕССИИ: обновляем остальные сервисные страницы по ПЛЕЙБУКУ** (раздел в `docs/HANDOFF.md`). Эталон — `hormone` (полностью доведён, desktop+mobile). Очередь: weight-loss, peptide, emsculpt, exomind, emface, emsella, ivtherapy, bloodwork, regenerative, sexualhealth. Спроси меня, с какой страницы начать (или предложи порядок). Для КАЖДОЙ страницы: сними геометрию секций из Figma → desktop (`lg:aspect-[W/H]` + vw-clamp шрифты, картинки/квадрат/свап по дизайну) → mobile (stacked hero, full-bleed фото, body font floor 16, порядок секций, зазоры) → аудит CTA против Figma → реальные ассеты (curl) → верификация Playwright + патч. Прочие направления (формы→backend, комплаенс, лицензии, домен, SEO) — в бэклоге, только если я попрошу.
>
> Мелкие незакрытые флаги из сессии 17 (если зайдёт речь): заголовок «We are located…» в `LocationBlock` (About/Contact) всё ещё капс-Marcellus (на главной уже Manrope по Figma); `heroTitle` страницы regenerative = «PRP & ALLOGRAFT» (меню/футер уже «Regenerative Medicine»); гор. скролл на 320px от картинки мозга в `NewIn2026`.
>
...
>
> Что готово: главная + все 11 specialty-страниц + блог (`/blog` + статьи) + About/Contact/Glossary. Мобильный адаптив + drill-down меню + десктоп sticky-bar/mega-menu сделаны. SEO-фундамент (JSON-LD, OG, canonical, sitemap, favicon). Главная выровнена по Figma. **`hormone` полностью доведён до Figma 1:1 (desktop+mobile, сессия 18) — эталон для остальных сервисных страниц.** **Сессия 17:** Suite 345 в адресе везде (+ JSON-LD), новые часы (Mon–Fri 10–6, Sat By Appointment, Sun Closed) в сворачиваемом виджете `HoursWidget.astro`, реальные соцсети + Cherry Financing в футере, «PRP & Allograft» → «Regenerative Medicine». Данные клиники в `src/data/site.ts` теперь реальные (адрес/часы/соцсети); заглушки остались только `geo` и прод-домен. Билд = **25 страниц**. Деплой — Cloudflare Pages.
>
