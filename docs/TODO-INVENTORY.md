...
**Шрифт Optima** — клиент в сессии 4 принял юр.риск использовать `Optima.ttc`
из системы macOS (лицензия Apple/Linotype) как self-hosted веб-шрифт. Web-
лицензия у Monotype не куплена ($35-90/год). Если потом захочется купить —
заменить файлы в `public/fonts/` на лицензированные woff2, frontend не
меняется.
